...
<div class="sm:hidden space-y-3">
    @foreach($this->weekDays as $day)
        @php
            $dayEvents = $this->weekEvents->filter(fn($e) => $e->start_time->format('Y-m-d') === $day['date']);
        @endphp
        <div class="border border-zinc-200 dark:border-zinc-700 rounded-lg overflow-hidden {{ $day['isToday'] ? 'ring-2 ring-blue-500' : '' }}">
            <!-- Day header -->
            <div class="flex items-center justify-between px-3 py-2 {{ $day['isToday'] ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-zinc-50 dark:bg-zinc-800' }}">
                <div class="flex items-center gap-2">
                    <span class="text-xs font-medium text-zinc-500 dark:text-zinc-400">{{ $day['dayName'] }}</span>
                    <span class="text-base font-semibold {{ $day['isToday'] ? 'text-blue-600 dark:text-blue-400' : 'text-zinc-900 dark:text-white' }}">{{ $day['day'] }}</span>
                </div>
                <a href="{{ route('admin.calendar.create', ['date' => $day['date']]) }}" wire:navigate
            ...
            @if($dayEvents->isNotEmpty())
                <div class="p-2 space-y-1">
                    @foreach($dayEvents->sortBy('start_time') as $event)
                        <a href="{{ route('admin.calendar.edit', $event) }}" wire:navigate
                           class="flex items-center gap-2 p-2 rounded text-sm transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                           style="border-left: 3px solid {{ $event->color ?? '#3b82f6' }}">
                            <span class="text-xs text-zinc-500 dark:text-zinc-400 w-10">{{ $event->start_time->format('H:i') }}</span>
                            <span class="font-medium text-zinc-900 dark:text-white truncate flex-1">{{ $event->title }}</span>
                        </a>
                    @endforeach
                </div>
            @endif
