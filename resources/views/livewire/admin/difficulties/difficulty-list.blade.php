    ...
    <header class="max-w-4xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12 pb-6">
        <div class="flex items-center sm:items-end justify-between mb-6 pb-4 border-b border-[#E9E9E7] dark:border-[#3E3E3A]">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-[#37352F] dark:text-[#EFEFED]">Dificultades</h1>
                    <p class="text-xs sm:text-sm text-[#9B9A97]">Administra los niveles de dificultad y puntos.</p>
                </div>
            </div>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6">
        @if($difficulties->isEmpty())
            ...
            <div class="border border-[#E9E9E7] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#252525] p-8 sm:p-12 text-center">
                ...
                <h3 class="text-base sm:text-lg font-bold text-[#37352F] dark:text-[#EFEFED] mb-2">No hay dificultades todavía</h3>
        ...
        @else
            ...
            <div class="hidden md:block border border-[#E9E9E7] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#252525] overflow-hidden">
                ...
                <div class="flex text-[11px] font-semibold text-[#9B9A97] uppercase tracking-wide bg-[#F7F7F5] dark:bg-[#1F1F1F] border-b border-[#E9E9E7] dark:border-[#3E3E3A]">
                    ...
                    <div class="w-32 px-4 py-2 border-r border-[#E9E9E7] dark:border-[#3E3E3A] text-center">XP Reward</div>
                ...
                @foreach($difficulties as $difficulty)
                    <div class="group flex items-center text-sm border-b border-[#E9E9E7] dark:border-[#3E3E3A] hover:bg-[#F7F7F5] dark:hover:bg-[#1F1F1F] transition h-12" data-id="{{ $difficulty->id }}">
                        ...
                        <div class="w-32 px-4 text-center">
                            <span class="px-2 py-1 rounded text-xs font-semibold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">
                                ⭐ {{ $difficulty->points }} XP
                            </span>
                        </div>
                        
                        <div class="w-24 px-4 text-center text-[#787774] dark:text-[#9B9A97]">
                            {{ $difficulty->habits_count }}
                        </div>
                        
                        <div class="w-20 px-4 flex justify-center">
                            <button wire:click="toggleActive({{ $difficulty->id }})"
                                    class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors
                                        {{ $difficulty->is_active ? 'bg-[#2383E2]' : 'bg-[#E3E2E0] dark:bg-[#3E3E3A]' }}">
                                <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform
                                    {{ $difficulty->is_active ? 'translate-x-5' : 'translate-x-0.5' }}"></span>
                            </button>
                        </div>
                    ...
                    <div class="bg-white dark:bg-[#252525] border border-[#E9E9E7] dark:border-[#3E3E3A] rounded-lg p-4">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-2">
                                <span class="text-xl">{{ $difficulty->icon ?? '⭐' }}</span>
                                <span class="font-medium text-[#37352F] dark:text-[#EFEFED]">{{ $difficulty->name }}</span>
                            </div>
                            <button wire:click="toggleActive({{ $difficulty->id }})"
                                    class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors
                                        {{ $difficulty->is_active ? 'bg-[#2383E2]' : 'bg-[#E3E2E0] dark:bg-[#3E3E3A]' }}">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="text-[#787774] dark:text-[#9B9A97]">{{ $difficulty->habits_count }} hábitos</span>
                        </div>
                        
                        <div class="flex items-center justify-between mt-3 pt-3 border-t border-[#E9E9E7] dark:border-[#3E3E3A]">
                            <div class="flex items-center gap-1">
                                @if($index > 0)
                                    <button wire:click="moveUp({{ $difficulty->id }})" class="p-1.5 text-[#9B9A97] hover:bg-[#EFEFED] dark:hover:bg-[#2A2A2A] rounded">↑</button>
                                @endif
                                @if($index < count($difficulties) - 1)
                                    <button wire:click="moveDown({{ $difficulty->id }})" class="p-1.5 text-[#9B9A97] hover:bg-[#EFEFED] dark:hover:bg-[#2A2A2A] rounded">↓</button>
                                @endif
                            </div>
                ...
                <div class="p-5">
                    ...
                    @else
                        <p class="text-sm text-[#37352F] dark:text-[#EFEFED] mb-2">
                            ¿Eliminar <strong>"{{ $difficultyToDeleteName }}"</strong>?
                        </p>
