<div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
    <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-4">
            <h3 class="font-bold text-slate-800 text-lg">Misiones de Hoy</h3>
            <div class="flex bg-gray-100 p-1 rounded-lg">
                <button 
                    wire:click="setFilter('pending')"
                    wire:loading.attr="disabled"
                    class="px-3 py-1 text-xs font-medium transition-all {{ $filter === 'pending' ? 'bg-white shadow-sm rounded-md font-bold text-slate-800' : 'text-slate-500 hover:text-slate-700' }} disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button 
                    ...
                    wire:loading.attr="disabled"
                    class="px-3 py-1 text-xs font-medium transition-all {{ $filter === 'all' ? 'bg-white shadow-sm rounded-md font-bold text-slate-800' : 'text-slate-500 hover:text-slate-700' }} disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    Todas
                </button>
            </div>
                ...
                @foreach($habits as $habit)
                    <div class="flex items-center justify-between p-4 hover:bg-gray-50 rounded-2xl transition-colors border border-transparent hover:border-gray-100 group cursor-pointer">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h4 class="font-bold text-slate-800 text-sm {{ $habit->isCompletedToday() ? 'line-through opacity-60' : '' }}">
                                    {{ $habit->name }}
                                </h4>
                        ...
                        <div class="flex items-center gap-6">
                            @if($habit->difficulty)
                                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold 
                                    {{ $habit->difficulty->value === 'easy' ? 'bg-green-50 text-green-600 border border-green-100' : '' }}
                                    {{ $habit->difficulty->value === 'medium' ? 'bg-orange-50 text-orange-600 border border-orange-100' : '' }}
                                    {{ $habit->difficulty->value === 'hard' || $habit->difficulty->value === 'epic' ? 'bg-red-50 text-red-600 border border-red-100' : '' }}">
                            ...
                            @endif
                            <span class="text-xs text-slate-500">{{ $habit->points_reward }} XP</span>
                        </div>
                    </div>
                @endforeach
            </div>
